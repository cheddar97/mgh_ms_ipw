...
data <- data |>
  mutate(
    smoking = case_when(
      str_detect(smoking, "^fo") ~ "no",
      str_detect(smoking, "^n")  ~ "no",
      str_detect(smoking, "^y")  ~ "yes",
      str_detect(smoking, "day")  ~ "yes",
      str_detect(smoking, "stigma")  ~ NA,
      TRUE ~ smoking
    )
  )
```
...
## Duration of MS Diagnosis

```{r}
data <- data |> mutate(ms.duration = age - age.at.ms.diagnosis)
```
...
## Current Therapy Cycles

* Clean the therapy cycles.
* Not of much use though, as the number of cycles may differ significantly in the control group.

```{r}
data <- data |>
  mutate(
    no.therapy.cycles = str_remove(no.therapy.cycles, "at least "),
    no.therapy.cycles = str_replace(no.therapy.cycles, " \\(.*", ""),
    no.therapy.cycles = case_when(
      no.therapy.cycles == "unknown" | no.therapy.cycles == "na" ~ NA,
      str_detect(no.therapy.cycles, "^0") ~ "1",
      str_detect(no.therapy.cycles, "[a-z]") ~ NA,
      TRUE ~ no.therapy.cycles
    ),
    no.therapy.cycles = as.numeric(no.therapy.cycles),
    no.therapy.cycles = round(no.therapy.cycles, digits = 0),
    no.therapy.cycles = as.integer(no.therapy.cycles),
  )
```
...
data <- data |>
  mutate(
    infection.binary = case_when(
      ...
      is.na(infections.on.current.therapy) ~ "0",
      TRUE ~ "1"
    ),
...
This is also done for the infection subtype data. `NA` is assumed to be `0`.
  ...
  mutate(
    across(
      c(
        infection.uti,
        infection.covid,
        infection.lower.resp,
        infection.upper.resp,
        infection.softtissue,
        infection.sepsis,
        infection.other
      ),
      function(x) {
